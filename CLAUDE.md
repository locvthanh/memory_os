...
`src/scenes2d/civil-war.data.js` is generated by `scripts/build_civil_war_2d.py`
(us-atlas geometry projected onto the Albers conic, plus the hand-authored
rivers, front lines and campaign arrows). **Never hand-edit it** — re-run the
script.

That used to be two files. `the-office` was the other: a Blender generator read
this repo and baked a corridor into a glb plus a `the-office.data.js`. It was
replaced on 2026-09-13 by a runtime build, because a generated room whose
generator has to be remembered is a room that goes stale — twice a finished
scene had no door because nobody re-ran it. See **Scenes with no model** below.
...
assembled in the browser plus `update` / `pick` / `dispose`. Four scenes do this. Three because their content is live and a model
exported last night would be yesterday's; the fourth because its content is
the repo itself:
...
- `cafef-vn` — cafef.vn as a trading floor, with live market numbers on the
  board (`src/scenesjs/cafef/`).
- `the-office` — **the Back Office**, and the one that is not about live
  content at all (`src/scenesjs/office/`). It is about the repo. On every open
  it reads `src/scenes/index.js` for the registered scenes, fetches every other
  `src/scenes/*.js` AS TEXT and scans the `link:` edges, works out which scenes
  nothing points at, and lays out a corridor with one door per orphan — as many
  bays as that needs. Its own files are skipped in the scan, or a room that
  links to every orphan would count its own edges and empty the corridor. The
  sources are fetched as text rather than imported because importing thirty
  configs would drag in every procedural scene's whole implementation. Unlike
  the other three it DOES have loci: `loci: []` in the config is filled in by
  `build` before it resolves, which is legal because `Viewer` reads
  `config.loci` only after the build promise settles. Add a note to `NOTES` in
  `src/scenesjs/office/index.js` when a door earns real words; never hand-add a
  locus.

The three live ones set `loci: []`, so the viewer drops the transport bar and the locus panel
